...
(function (g) {
  ...
  const L = {
    ...
    TABLES: [[-7.0, 2.6], [-7.0, 6.4], [7.0, 2.6], [7.0, 6.4]],
    COLUMNS: [[-4.2, 0.8], [-4.2, 3.6], [-4.2, 6.4], [4.2, 0.8], [4.2, 3.6], [4.2, 6.4]],
    LANTERN: { x0: -8.2, x1: 8.2, z0: -0.3, z1: 8.3, y0: 3.0, y1: 4.85 },
  ...
  /* opts.lite drops decorative geometry — the world still reads correctly but
     fits well inside a 16-bit index buffer for drivers without
     OES_element_index_uint, and costs far less to draw on weak hardware. */
  function build(opts) {
    ...
    /* ══════════════════════════ interior ══════════════════════════ */
    /* The room is shaped, not just surfaced: a checkerboard floor, a colonnade
       splitting the corridor from the seating bays, and a tall central lantern
       that lifts the ceiling from 3.0 m over the perimeter to 4.85 m over the
       dining room. Sightlines are kept open across the pass so the cook line is
       part of the room rather than hidden behind a wall of signage. */
    ...
    const DADO_H = 1.06;
    const LN = L.LANTERN;
    const SOF = LN.y0;                       // perimeter soffit height

    /* ---- floors ---- */
    // checkerboard dining floor, inside a red terrazzo border
    op.tiled([-W + 0.9, 0, D - 0.9], [W - 0.9, 0, D - 0.9], [W - 0.9, 0, cz + 0.9], [-W + 0.9, 0, cz + 0.9],
      { tile: T.CHECKER_BW, rep: 1.6, tint: C.white, flat: 1.0 });
    const border = (x0, x1, z0, z1) => op.tiled([x0, 0, z1], [x1, 0, z1], [x1, 0, z0], [x0, 0, z0],
      { tile: T.TERRAZZO_RED, rep: 1.2, tint: C.white, flat: 1.0 });
    border(-W, W, cz, cz + 0.9);
    border(-W, W, D - 0.9, D);
    border(-W, -W + 0.9, cz + 0.9, D - 0.9);
    border(W - 0.9, W, cz + 0.9, D - 0.9);
    op.tiled([-W, 0, cz], [W, 0, cz], [W, 0, -D], [-W, 0, -D],
      { tile: T.QUARRY, rep: 0.95, tint: C.white, flat: 0.94 });
    op.decal(0, 0.016, 7.9, 4.6, 1.9, { tile: T.CHECKER, rep: 0.5, tint: C.white, flat: 0.95 });

    /* ---- ceiling: perimeter soffit + raised lantern ---- */
    const soffit = (x0, x1, z0, z1) => op.tiled([x0, SOF, z0], [x1, SOF, z0], [x1, SOF, z1], [x0, SOF, z1],
      { tile: T.CEIL, rep: 1.5, tint: C.white, flat: 1.45 });
    soffit(-W, W, LN.z1, D);
    soffit(-W, W, -D, LN.z0);
    soffit(-W, LN.x0, LN.z0, LN.z1);
    soffit(LN.x1, W, LN.z0, LN.z1);
    ...
    // lantern reveal walls, facing down into the room
    const lanternWall = { tile: T.PANEL, rep: 1.6, tint: C.cream, flat: 1.25 };
    wallIn(LN.x0, SOF, LN.z0, LN.x0, LN.y1, LN.z1, lanternWall);
    wallIn(LN.x1, SOF, LN.z0, LN.x1, LN.y1, LN.z1, lanternWall);
    wallIn(LN.x0, SOF, LN.z0, LN.x1, LN.y1, LN.z0, lanternWall);
    wallIn(LN.x0, SOF, LN.z1, LN.x1, LN.y1, LN.z1, lanternWall);
    // lantern ceiling
    op.tiled([LN.x0, LN.y1, LN.z0], [LN.x1, LN.y1, LN.z0], [LN.x1, LN.y1, LN.z1], [LN.x0, LN.y1, LN.z1],
      { tile: T.CEIL, rep: 1.5, tint: C.white, flat: 1.5 });
    // red reveal trim around the opening, with a chrome lip
    [[LN.x0 - 0.09, LN.x0 - 0.09, LN.z0, LN.z1, 0.18, LN.z1 - LN.z0],
     [LN.x1 + 0.09, LN.x1 + 0.09, LN.z0, LN.z1, 0.18, LN.z1 - LN.z0]].forEach(([x]) => {
      op.box(x, SOF + 0.13, (LN.z0 + LN.z1) / 2, 0.3, 0.26, LN.z1 - LN.z0 + 0.36, { tile: T.BLANK, tint: C.red, flat: 1.05 });
      op.box(x, SOF - 0.03, (LN.z0 + LN.z1) / 2, 0.36, 0.09, LN.z1 - LN.z0 + 0.42, { tile: T.CHROME, tint: C.chrome });
    });
    [LN.z0 - 0.09, LN.z1 + 0.09].forEach(z => {
      op.box(0, SOF + 0.13, z, LN.x1 - LN.x0 + 0.36, 0.26, 0.3, { tile: T.BLANK, tint: C.red, flat: 1.05 });
      op.box(0, SOF - 0.03, z, LN.x1 - LN.x0 + 0.42, 0.09, 0.36, { tile: T.CHROME, tint: C.chrome });
    });
    // exposed beams spanning the lantern
    for (let z = LN.z0 + 1.4; z < LN.z1; z += (LITE ? 4.2 : 2.1)) {
      op.box(0, LN.y1 - 0.19, z, LN.x1 - LN.x0, 0.34, 0.28, { tile: T.BLANK, tint: C.red, flat: 1.05 });
      op.box(0, LN.y1 - 0.38, z, LN.x1 - LN.x0, 0.07, 0.34, { tile: T.CHROME, tint: C.chrome });
    }
    // uplight strip washing the lantern walls
    [[LN.x0 + 0.3, 0], [LN.x1 - 0.3, 0]].forEach(([x]) => {
      em.box(x, SOF + 0.16, (LN.z0 + LN.z1) / 2, 0.14, 0.05, LN.z1 - LN.z0 - 0.6, { tile: T.NEON, tint: C.white, flat: 1 });
    });
    // pendant cluster hanging in the lantern
    (LITE ? [[0, 4.0]] : [[-2.4, 2.4], [2.4, 2.4], [-2.4, 5.6], [2.4, 5.6], [0, 4.0]]).forEach(([x, z]) => {
      const drop = 1.5;
      op.cyl(x, LN.y1 - drop, z, 0.022, drop, 6, { tile: T.CHROME, tint: C.chrome, flat: 0.95 });
      op.cyl(x, LN.y1 - drop - 0.34, z, 0.4, 0.36, 12, { tile: T.BLANK, tint: C.red, flat: 1.08, rTop: 0.14 });
      op.cyl(x, LN.y1 - drop - 0.4, z, 0.36, 0.07, 12, { tile: T.CHROME, tint: C.chrome, flat: 1.05 });
      em.cyl(x, LN.y1 - drop - 0.43, z, 0.31, 0.05, 12, { tile: T.NEON, tint: C.white, flat: 1 });
    });
    // recessed downlights in the soffit ring
    (LITE ? [[-10.4, 5.0], [10.4, 5.0], [0, 8.8], [0, -0.9]]
          : [[-10.4, 1.5], [-10.4, 5.0], [-10.4, 8.0], [10.4, 1.5], [10.4, 5.0], [10.4, 8.0],
             [-6, 8.8], [0, 8.8], [6, 8.8], [-6, -0.9], [0, -0.9], [6, -0.9]]).forEach(([x, z]) => {
      op.box(x, SOF - 0.04, z, 0.66, 0.08, 0.66, { tile: T.CHROME, tint: C.chrome, flat: 1.0 });
      em.box(x, SOF - 0.1, z, 0.54, 0.04, 0.54, { tile: T.BLANK, tint: 0xfff8ea, flat: 1 });
    });
    // kitchen strip lights under the soffit
    for (let x = -9; x <= 9; x += (LITE ? 7.2 : 3.6)) {
      op.box(x, SOF - 0.05, -5, 0.34, 0.1, 4.6, { tile: T.CHROME, tint: C.chrome, flat: 1.0 });
      em.box(x, SOF - 0.12, -5, 0.26, 0.04, 4.4, { tile: T.BLANK, tint: 0xfff8ea, flat: 1 });
    }

    /* ---- walls ---- */
    const wallW = { tile: T.PANEL, rep: 1.8, tint: C.cream, flat: 1.0 };
    const wallK = { tile: T.TILEWALL, rep: 1.05, tint: C.white, flat: 1.0 };
    const dado  = { tile: T.CHECKER, rep: 0.53, tint: C.white, flat: 1.0 };

    wallIn(-W, 0, -D, W, SOF, -D, wallK);
    wallIn(-W, 0, -D, -W, SOF, cz, wallK);
    wallIn(W, 0, -D, W, SOF, cz, wallK);
    op.box(0, SOF - 0.22, -D + 0.05, W * 2, 0.44, 0.1, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
    [[-W + 0.05, 1], [W - 0.05, -1]].forEach(([x]) => {
      op.box(x, SOF - 0.22, (cz - D) / 2, 0.1, 0.44, D + cz, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
    });

    [[-W, 1], [W, -1]].forEach(([x, sg]) => {
      wallIn(x, 0, cz, x, DADO_H, D, dado);
      wallIn(x, DADO_H, cz, x, winY1, D, wallW);
      wallIn(x, winY1, cz, x, SOF, D, wallW);
      op.box(x - sg * 0.055, DADO_H + 0.04, (cz + D) / 2, 0.13, 0.11, D - cz, { tile: T.CHROME, tint: C.chrome });
      op.box(x - sg * 0.05, SOF - 0.26, (cz + D) / 2, 0.11, 0.5, D - cz, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
      op.box(x - sg * 0.06, SOF - 0.53, (cz + D) / 2, 0.14, 0.08, D - cz, { tile: T.CHROME, tint: C.chrome });
    ...
    [[-W, -dw], [dw, W]].forEach(([a, b]) => {
      ...
      wallIn(a, winY1, D, b, SOF, D, wallW);
      ...
      gl.quad([a + 0.35, winY0, D - 0.02], [b - 0.35, winY0, D - 0.02],
              ...
              { tile: T.GLASS, tint: C.white, flat: 1 });
      op.box((a + b) / 2, DADO_H + 0.04, D - 0.055, b - a, 0.11, 0.13, { tile: T.CHROME, tint: C.chrome });
      op.box((a + b) / 2, SOF - 0.26, D - 0.05, b - a, 0.5, 0.11, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
      op.box((a + b) / 2, SOF - 0.53, D - 0.06, b - a, 0.08, 0.14, { tile: T.CHROME, tint: C.chrome });
      for (let x = a + 2.2; x < b - 0.5; x += 2.2) {
        op.box(x, (winY0 + winY1) / 2, D - 0.055, 0.11, winY1 - winY0, 0.11, { tile: T.CHROME, tint: C.steel });
      }
    });
    wallIn(-dw, 2.35, D, dw, SOF, D, wallW);
    op.box(0, SOF - 0.26, D - 0.05, dw * 2, 0.5, 0.11, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
    ...
    /* ---- colonnade: chunky columns splitting corridor from seating ---- */
    L.COLUMNS.forEach(([x, z]) => {
      op.box(x, 0.13, z, 1.0, 0.26, 1.0, { tile: T.TERRAZZO_RED, rep: 0.9, tint: C.white, flat: 1.0 });
      op.box(x, 0.29, z, 0.86, 0.08, 0.86, { tile: T.CHROME, tint: C.chrome });
      op.cyl(x, 0.33, z, 0.3, SOF - 0.72, 12, { tile: T.PANEL, tint: C.white, flat: 1.05 });
      op.cyl(x, 0.33, z, 0.315, 0.5, 12, { tile: T.CHECKER, rep: 0.34, tint: C.white, flat: 1.0 });
      op.cyl(x, SOF - 0.42, z, 0.35, 0.14, 12, { tile: T.CHROME, tint: C.chrome, flat: 1.08 });
      op.box(x, SOF - 0.19, z, 0.86, 0.34, 0.86, { tile: T.PANEL_RED, tint: C.white, flat: 1.08 });
      op.box(x, SOF - 0.02, z, 0.96, 0.08, 0.96, { tile: T.CHROME, tint: C.chrome });
      solid(x - 0.5, x + 0.5, z - 0.5, z + 0.5);
      round.push({ x, z, r: 0.66 });
      shade(x, z, 1.7, 1.7, 0.8);
    });

    /* ---- service counter, open to the kitchen ---- */
    op.box(cmid, ch / 2, cz, clen, ch, cd, { tile: T.PANEL, tint: C.white, rep: 1.4 });
    op.box(cmid, 0.36, cz + cd / 2 + 0.02, clen, 0.64, 0.06, { tile: T.CHECKER, rep: 0.53, tint: C.white, flat: 1.0 });
    op.box(cmid, 0.7, cz + cd / 2 + 0.03, clen, 0.09, 0.1, { tile: T.CHROME, tint: C.chrome });
    op.box(cmid, 0.05, cz + cd / 2 + 0.03, clen, 0.1, 0.1, { tile: T.CHROME, tint: C.steel });
    op.box(cmid, ch + 0.05, cz, clen + 0.2, 0.1, cd + 0.2, { tile: T.MARBLE, rep: 1.6, tint: C.white, flat: 1.05 });
    op.box(cmid, ch + 0.11, cz, clen + 0.24, 0.04, cd + 0.24, { tile: T.CHROME, tint: C.chrome });
    op.box(cmid, 0.94, cz + cd / 2 + 0.16, clen, 0.05, 0.24, { tile: T.CHROME, tint: C.chrome });
    [-8.4, -5.6, -2.8, 0, 2.8, 5.4].forEach(x =>
      op.box(x, 0.8, cz + cd / 2 + 0.22, 0.06, 0.24, 0.06, { tile: T.CHROME, tint: C.steel }));
    solid(L.COUNTER_X0 - 0.1, L.COUNTER_X1 + 0.1, cz - cd / 2 - 0.1, cz + cd / 2 + 0.35);
    shade(cmid, cz + 0.5, clen + 1.2, 2.6, 0.85);

    [-2.0, -5.2].forEach(x => {
      op.box(x, ch + 0.24, cz - 0.05, 0.48, 0.24, 0.38, { tile: T.PANEL, tint: C.dark, flat: 0.95 });
      op.box(x, ch + 0.37, cz - 0.05, 0.52, 0.04, 0.42, { tile: T.CHROME, tint: C.chrome });
      op.box(x, ch + 0.44, cz - 0.14, 0.46, 0.32, 0.07, { tile: T.PANEL, tint: C.dark, flat: 0.95 });
      em.box(x, ch + 0.44, cz - 0.185, 0.38, 0.25, 0.02, { tile: T.MENU, tint: C.white, flat: 1 });
      em.box(x, ch + 0.24, cz + 0.145, 0.32, 0.13, 0.02, { tile: T.MENU, tint: 0xdde8ff, flat: 1 });
    });

    op.box(3, ch + 1.06, cz - 0.16, 2.5, 0.1, 0.44, { tile: T.CHROME, tint: C.chrome });
    [2.1, 3.9].forEach(x => op.box(x, ch + 1.38, cz - 0.16, 0.05, 0.58, 0.05, { tile: T.CHROME, tint: C.steel }));
    [2.3, 3.0, 3.7].forEach(x => em.box(x, ch + 1.0, cz - 0.16, 0.5, 0.05, 0.3, { tile: T.NEON, tint: C.white, flat: 1 }));
    op.box(3, ch + 0.14, cz + 0.06, 1.9, 0.06, 0.45, { tile: T.CHROME, tint: C.chrome });

    /* menu boards hung from the soffit, kept off to the order side so the
       cook line stays visible across the pass */
    const board = (x, w) => {
      op.box(x, SOF - 0.62, cz - 0.42, w + 0.16, 0.98, 0.14, { tile: T.CHROME, tint: C.chrome });
      op.box(x, SOF - 0.62, cz - 0.37, w, 0.84, 0.04, { tile: T.PANEL, tint: C.dark, flat: 0.9 });
      em.quad([x - w / 2 + 0.05, SOF - 1.0, cz - 0.345], [x + w / 2 - 0.05, SOF - 1.0, cz - 0.345],
              [x + w / 2 - 0.05, SOF - 0.24, cz - 0.345], [x - w / 2 + 0.05, SOF - 0.24, cz - 0.345],
              { tile: T.MENU, tint: C.white, flat: 1 });
      op.box(x, SOF - 0.1, cz - 0.42, w + 0.22, 0.14, 0.2, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
    };
    board(-6.5, 2.9);
    board(-3.1, 2.9);
    em.box(-8.6, SOF - 0.62, cz - 0.32, 0.9, 0.9, 0.06, { tile: T.LOGO, tint: C.white, flat: 1 });

    /* hanging double-sided ORDER HERE / PICK UP sign over the pass */
    const hangSign = (x, tile, w) => {
      [-1, 1].forEach(o => op.cyl(x + o * (w / 2 - 0.2), SOF - 0.72, cz - 0.1, 0.022, 0.72, 6, { tile: T.CHROME, tint: C.chrome, flat: 0.95 }));
      op.box(x, SOF - 1.0, cz - 0.1, w, 0.56, 0.1, { tile: T.CHROME, tint: C.chrome });
      em.box(x, SOF - 1.0, cz - 0.16, w - 0.14, 0.42, 0.02, { tile, tint: C.white, flat: 1 });
      em.box(x, SOF - 1.0, cz - 0.04, w - 0.14, 0.42, 0.02, { tile, tint: C.white, flat: 1 });
    };
    hangSign(-3.6, T.SIGN_MENU, 2.6);
    hangSign(3.0, T.SIGN_OPEN, 2.4);
    ...
    L.BOOTHS.forEach(([bx, bz]) => {
      ...
      [-1, 1].forEach(d => {
        const sz = bz + d * 1.05;
        op.box(bx, 0.45, sz, 2.3, 0.15, 0.64, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
        op.box(bx, 0.535, sz, 2.34, 0.05, 0.68, { tile: T.CHROME, tint: C.chrome });
        op.box(bx, 0.22, sz, 2.2, 0.44, 0.5, { tile: T.PANEL, tint: C.white, flat: 0.94 });
        op.box(bx, 0.88, sz + d * 0.43, 2.3, 1.0, 0.2, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
        op.box(bx, 1.4, sz + d * 0.43, 2.36, 0.09, 0.26, { tile: T.CHROME, tint: C.chrome });
        op.box(bx, 0.63, sz + d * 0.43, 2.3, 0.07, 0.25, { tile: T.CHROME, tint: C.chrome });
        shade(bx, sz, 2.6, 1.1, 0.84);
        round.push({ x: bx, z: sz, r: 0.75 });
      });
      op.box(bx, 0.72, bz, 1.9, 0.08, 0.88, { tile: T.MARBLE, rep: 1.4, tint: C.white, flat: 1.05 });
      op.box(bx, 0.775, bz, 1.96, 0.035, 0.94, { tile: T.CHROME, tint: C.chrome });
      op.cyl(bx, 0, bz, 0.07, 0.7, 8, { tile: T.CHROME, tint: C.steel, flat: 0.9 });
      op.cyl(bx, 0, bz, 0.28, 0.05, 10, { tile: T.CHROME, tint: C.steel, flat: 0.84, closeBottom: true });
      op.box(bx + 0.62, 0.82, bz, 0.17, 0.11, 0.32, { tile: T.CHROME, tint: C.chrome });
      op.cyl(bx + 0.62, 0.86, bz - 0.07, 0.032, 0.12, 6, { tile: T.BLANK, tint: C.red, flat: 1.08 });
      op.cyl(bx + 0.62, 0.86, bz + 0.07, 0.032, 0.12, 6, { tile: T.BLANK, tint: C.yellow, flat: 1.08 });
      // aisle-end divider with a glazed upper panel
      op.box(ix, 0.62, bz, 0.18, 1.24, 2.9, { tile: T.PANEL, tint: C.white, flat: 1.0 });
      op.box(ix, 1.27, bz, 0.24, 0.1, 2.96, { tile: T.CHROME, tint: C.chrome });
      op.box(ix, 1.62, bz, 0.1, 0.6, 2.9, { tile: T.CHECKER, rep: 0.42, tint: C.white, flat: 1.0 });
      op.box(ix, 1.95, bz, 0.24, 0.1, 2.96, { tile: T.PANEL_RED, tint: C.white, flat: 1.08 });
      solid(ix - 0.14, bx + sg * 1.5, bz - 1.55, bz + 1.55, 1.2);
      round.push({ x: ix, z: bz, r: 0.7 });
      shade(bx, bz, 3.0, 3.4, 0.8);
    });

    /* ---- round tables in the bays ---- */
    L.TABLES.forEach(([x, z]) => {
      op.cyl(x, 0.72, z, 0.62, 0.07, 12, { tile: T.MARBLE, rep: 1.2, tint: C.white, flat: 1.05, closeBottom: true });
      op.cyl(x, 0.785, z, 0.65, 0.035, 12, { tile: T.CHROME, tint: C.chrome, flat: 1.0 });
      op.cyl(x, 0.05, z, 0.09, 0.68, 8, { tile: T.CHROME, tint: C.steel, flat: 0.9 });
      op.cyl(x, 0, z, 0.34, 0.07, 10, { tile: T.CHROME, tint: C.steel, flat: 0.84, closeBottom: true });
      op.box(x, 0.85, z, 0.15, 0.13, 0.28, { tile: T.CHROME, tint: C.chrome });
      ...
      [[-0.95, 0], [0.95, 0], [0, -0.95], [0, 0.95]].forEach(([ox, oz]) => {
        ...
        op.box(sx, 0.44, sz, 0.48, 0.11, 0.48, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
        op.box(sx, 0.505, sz, 0.52, 0.04, 0.52, { tile: T.CHROME, tint: C.chrome });
        op.cyl(sx, 0, sz, 0.06, 0.42, 8, { tile: T.CHROME, tint: C.steel, flat: 0.88 });
        op.cyl(sx, 0, sz, 0.2, 0.05, 8, { tile: T.CHROME, tint: C.steel, flat: 0.82, closeBottom: true });
        const bx2 = ox !== 0 ? Math.sign(ox) * 0.21 : 0, bz2 = oz !== 0 ? Math.sign(oz) * 0.21 : 0;
        op.box(sx + bx2, 0.76, sz + bz2, ox !== 0 ? 0.1 : 0.48, 0.54, oz !== 0 ? 0.1 : 0.48,
          { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
        op.box(sx + bx2, 1.04, sz + bz2, ox !== 0 ? 0.14 : 0.52, 0.06, oz !== 0 ? 0.14 : 0.52,
    ...
    (function lobbyDrinks() {
      const x = -9.4, z = 8.0;
      op.box(x, 0.45, z, 2.4, 0.9, 0.7, { tile: T.PANEL, tint: C.white, flat: 1.0 });
      op.box(x, 0.94, z, 2.5, 0.09, 0.8, { tile: T.MARBLE, rep: 1.4, tint: C.white, flat: 1.05 });
      op.box(x, 0.3, z + 0.37, 2.4, 0.5, 0.05, { tile: T.CHECKER, rep: 0.5, tint: C.white, flat: 1.0 });
      op.box(x, 1.76, z - 0.16, 2.3, 1.5, 0.5, { tile: T.PANEL, tint: C.dark, flat: 0.95 });
      em.quad([x - 1.1, 1.2, z + 0.095], [x + 1.1, 1.2, z + 0.095],
              [x + 1.1, 2.44, z + 0.095], [x - 1.1, 2.44, z + 0.095], { tile: T.SODA, tint: C.white, flat: 1 });
      op.box(x, 2.56, z - 0.16, 2.44, 0.16, 0.56, { tile: T.PANEL_RED, tint: C.white, flat: 1.08 });
      [-0.8, -0.27, 0.27, 0.8].forEach(o => {
        op.box(x + o, 1.17, z + 0.2, 0.11, 0.22, 0.16, { tile: T.CHROME, tint: C.chrome });
        op.box(x + o, 0.99, z + 0.16, 0.24, 0.03, 0.24, { tile: T.CHROME, tint: C.steel });
      });
    ...
    [[-4.6, 8.2], [4.6, 8.2]].forEach(([x, z]) => {
      op.box(x, 0.48, z, 1.5, 0.96, 0.6, { tile: T.PANEL, tint: C.white, flat: 1.0 });
      op.box(x, 1.0, z, 1.58, 0.09, 0.68, { tile: T.MARBLE, rep: 1.2, tint: C.white, flat: 1.05 });
      op.box(x, 0.3, z + 0.32, 1.5, 0.5, 0.05, { tile: T.CHECKER, rep: 0.5, tint: C.white, flat: 1.0 });
      op.box(x - 0.42, 1.13, z, 0.4, 0.16, 0.34, { tile: T.CHROME, tint: C.chrome });
      op.cyl(x + 0.1, 1.05, z, 0.055, 0.2, 6, { tile: T.BLANK, tint: C.red, flat: 1.08 });
      op.cyl(x + 0.3, 1.05, z, 0.055, 0.2, 6, { tile: T.BLANK, tint: C.yellow, flat: 1.08 });
      op.box(x, 1.6, z - 0.24, 1.2, 0.9, 0.1, { tile: T.CHROME, tint: C.chrome });
      em.box(x, 1.6, z - 0.3, 1.08, 0.78, 0.02, { tile: T.LOGO, tint: C.white, flat: 1 });
      solid(x - 0.8, x + 0.8, z - 0.36, z + 0.36, 1.2);
      shade(x, z, 2.2, 1.4, 0.85);
      round.push({ x, z, r: 0.85 });
    });

    [[-1.9, 8.4], [1.9, 8.4]].forEach(([x, z]) => {
      op.box(x, 0.5, z, 0.8, 1.0, 0.64, { tile: T.PANEL, tint: C.white, flat: 1.0 });
      op.box(x, 1.05, z, 0.86, 0.1, 0.7, { tile: T.PANEL_RED, tint: C.white, flat: 1.08 });
      op.box(x, 0.62, z + 0.33, 0.5, 0.42, 0.06, { tile: T.PANEL, tint: C.dark, flat: 0.85 });
      op.box(x, 0.2, z + 0.33, 0.8, 0.4, 0.05, { tile: T.CHECKER, rep: 0.5, tint: C.white, flat: 1.0 });
      solid(x - 0.43, x + 0.43, z - 0.37, z + 0.37, 1.1);
      shade(x, z, 1.4, 1.2, 0.85);
      round.push({ x, z, r: 0.55 });
    });

    [[-7.6, 5.4], [-7.6, 3.9]].forEach(([x, z]) => {
      op.box(x, 0.6, z, 0.56, 1.2, 0.34, { tile: T.PANEL, tint: C.dark, flat: 0.9 });
      op.box(x, 1.36, z, 0.72, 0.96, 0.26, { tile: T.CHROME, tint: C.chrome });
      op.box(x, 1.9, z, 0.78, 0.1, 0.32, { tile: T.PANEL_RED, tint: C.white, flat: 1.08 });
      em.quad([x - 0.29, 0.98, z + 0.135], [x + 0.29, 0.98, z + 0.135],
              [x + 0.29, 1.78, z + 0.135], [x - 0.29, 1.78, z + 0.135], { tile: T.MENU, tint: C.white, flat: 1 });
      solid(x - 0.36, x + 0.36, z - 0.22, z + 0.22, 1.6);
      shade(x, z, 1.2, 1.0, 0.85);
      round.push({ x, z, r: 0.5 });
    });

    /* framed wall art between the windows */
    [[-W + 0.1, 1, 0.2], [W - 0.1, -1, 0.2]].forEach(([x, sg, z]) => {
      op.box(x, 1.85, z, 0.1, 1.2, 1.6, { tile: T.CHROME, tint: C.chrome });
      em.quad(sg > 0 ? [x + 0.06, 1.35, z - 0.66] : [x - 0.06, 1.35, z + 0.66],
              sg > 0 ? [x + 0.06, 1.35, z + 0.66] : [x - 0.06, 1.35, z - 0.66],
              sg > 0 ? [x + 0.06, 2.35, z + 0.66] : [x - 0.06, 2.35, z - 0.66],
              sg > 0 ? [x + 0.06, 2.35, z - 0.66] : [x - 0.06, 2.35, z + 0.66],
              { tile: T.LOGO, tint: C.white, flat: 1 });
    });
    ...
    (function grill() {
      ...
      op.box(x, 2.32, z - 0.45, 3.5, 0.1, 0.8, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
      [-1.2, -0.4, 0.4, 1.2].forEach(o => op.cyl(x + o, 0.93, z + 0.62, 0.055, 0.06, 8, { tile: T.BLANK, tint: C.red, flat: 1.05 }));
    ...
    (function fryer() {
      ...
      op.box(x, 2.25, z - 0.4, 2.2, 0.1, 0.75, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
    ...
    (function drinks() {
      ...
      op.box(x, 2.34, z - 0.16, 1.8, 0.12, 0.66, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
    ...
    (function icecream() {
      ...
      op.box(x, 2.34, z, 1.18, 0.1, 0.92, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
    ...
    for (let i = 0; i < 4; i++) {
      ...
      op.box(x, 2.16, -8.35, 2.06, 0.12, 1.06, { tile: T.PANEL_RED, tint: C.white, flat: 1.05 });
    ...
    (function bin() {
      ...
      op.cyl(x, 0.9, z, 0.37, 0.1, 10, { tile: T.BLANK, tint: C.red, flat: 1.05 });
    ...
    op.cyl(-11.5, 0.9, -6.4, 0.11, 0.55, 8, { tile: T.BLANK, tint: C.red, flat: 1.05, closeBottom: true });
    ...
    op.box(-11.79, 1.55, -5.2, 0.04, 0.2, 0.06, { tile: T.BLANK, tint: C.green, flat: 1.05 });
    op.box(-11.79, 1.55, -5.2, 0.04, 0.06, 0.2, { tile: T.BLANK, tint: C.green, flat: 1.05 });
    ...
    [[-1.6, C.red], [-1.35, C.yellow], [-1.1, 0x8a6a3a]].forEach(([x, col]) => {
      op.cyl(x, 0.92, -2.95, 0.05, 0.2, 8, { tile: T.BLANK, tint: col, flat: 1.05, closeBottom: true });
    ...
    /* raised roof monitor covering the interior lantern */
    op.box(0, 4.95, 4.0, 17.4, 1.1, 9.4, { tile: T.STUCCO, rep: 2.0, tint: C.bone, skip: { bottom: true } });
    op.box(0, 5.42, 4.0, 17.9, 0.28, 9.9, { tile: T.ROOF, rep: 1.4, tint: C.red });
    op.box(0, 5.6, 4.0, 18.2, 0.14, 10.2, { tile: T.CHROME, tint: C.chrome });
    for (let x = -7.6; x <= 7.6; x += 3.8) {
      em.box(x, 4.9, 8.76, 2.2, 0.7, 0.05, { tile: T.MENU, tint: 0xbcd4e8, flat: 1 });
      op.box(x, 4.9, 8.72, 2.36, 0.84, 0.08, { tile: T.CHROME, tint: C.chrome });
    }
    ...
    op.box(0, 3.2, (canZ0 + canZ1) / 2, 19.0, 0.12, canZ1 - canZ0, { tile: T.PANEL, tint: C.white, flat: 1.4 });
    for (let i = 0; i < 8; i++) {
    ...
    /* sidewalk + patio deck */
    /* An apron around the building, not a slab under it — a full-footprint quad
       here sits above the interior floor at y=0 and paints over the whole dining
       room. */
    const walk = (x0, x1, z0, z1) => op.tiled([x0, 0.02, z1], [x1, 0.02, z1], [x1, 0.02, z0], [x0, 0.02, z0],
      { tile: T.CONCRETE, rep: 2.0, tint: C.white, flat: 0.95 });
    walk(-16, 16, EZ, canZ1 + 3.2);
    walk(-16, 16, -EZ - 3, -EZ);
    walk(-16, -EX, -EZ, EZ);
    walk(EX, 16, -EZ, EZ);
    ...
    const TREES = LITE
      ? [[-19, 14], [19, 14], [-21, -9], [21, -9]]
      : [[-19, 14], [-22, 6], [-19, -2], [-21, -9], [19, 14], [22, 6], [19, -2], [21, -9],
         [-24, 20], [24, 20], [-13, 20.5], [13, 20.5]];
    TREES.forEach(([x, z], i) => tree(x, z, 0.85 + (i % 3) * 0.16));
    if (!LITE) {
    ...
    for (let i = 0; i < (LITE ? 10 : 26); i++) {
      const a = (i / (LITE ? 10 : 26)) * Math.PI * 2 + rnd() * 0.22;
      const d = 52 + rnd() * 26;
      const h = 4 + rnd() * 13;
    ...
    for (let i = 0; i < (LITE ? 5 : 13); i++) {
      const a = (i / (LITE ? 5 : 13)) * Math.PI * 2 + rnd();
      const d = 34 + rnd() * 30;
